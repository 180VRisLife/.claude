Create a `.docs/plans/[feature-name]/parallelization.md` document breaking down Stream Deck plugin implementation into parallel tasks using `$CLAUDE_HOME/file-templates/parallelization.template.md`.
...
**If ≥5 Tier 1 tasks:**
Continue with parallel planning. If tasks have heavy dependencies or touch same plugin files extensively, warn: "Many dependencies limit parallelization. Expect some sequential bottlenecks."

### 3. Additional Research (if needed)
If planning documents lack details, read relevant plugin files. Quality is critical - mistakes cause implementation failures.

### 4. Create Parallel Plan
Break work into phases and tasks. Each task:
- **Brief**: Few plugin file changes
...
Files to Create
- /path/to/new/action.js
- /path/to/new/propertyInspector.html
...
[Minimal implementation instructions for basic Property Inspector, placeholder icon, etc.]
```

### 5. Plan Structure
Top of document includes:
- High-level explanation (3-4 sentences)
- Critically relevant plugin file paths
- Documentation paths

Organize phases:
- Phase 1: Foundation/core setup (Manifest updates, Action handlers, event structure)
- Phase 2+: Feature implementations (Property Inspectors, UI - parallel)
- Final: Integration, plugin validation
