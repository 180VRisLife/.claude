# Claude Prompting Guide for Stream Deck Plugin Development
...
<more_effective>
Your response will be deployed in a production plugin where stability is critical, so avoid unsafe operations that could cause crashes. Use proper error handling and validation instead.
</more_effective>
</example>
...
<more_effective>
Use the latest stable Stream Deck SDK APIs and ensure compatibility with current best practices.
</more_effective>
</example>
...
- Examine the existing plugin structure
...
<example>
<less_effective>
Analyze these plugin files and identify architectural improvements:
...
Analyze the plugin files above. Identify architectural patterns and recommend improvements for best practices.
</more_effective>
</example>
...
Organize multiple plugin files with clear metadata and boundaries:

<example>

<documents>
  <document>
    <source>src/actions/CounterAction.ts</source>
    <document_content>
      {{ACTION_CODE}}
    </document_content>
  </document>
  <document>
    <source>src/services/ApiService.ts</source>
    <document_content>
      {{SERVICE_CODE}}
    </document_content>
  </document>
...
Based on the plugin files above, provide a comprehensive architecture analysis with specific recommendations.
</example>

### 3. Ground Responses in Quotes

For accuracy in long plugin tasks, request relevant quotes before analysis:

<example>
You are analyzing plugin architecture to assist with optimization.

<documents>
  <document index="1">
    <source>src/actions/MainAction.ts</source>
    <document_content>
      {{ACTION_CODE}}
    </document_content>
  </document>
  <document index="2">
    <source>src/services/IntegrationService.ts</source>
    <document_content>
      {{SERVICE_CODE}}
    </document_content>
  </document>
...
1. First, extract relevant code quotes from the documents that relate to event handling. Place these in <relevant_quotes> tags.
...
<workflow>
  1. Use search and analysis tools to understand the plugin structure
