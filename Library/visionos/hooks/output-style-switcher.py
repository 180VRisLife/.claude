...
Modifies the outputStyle field in workspace .claude/settings.local.json (created from template during init).
"""
import json
import re
import sys
...
STYLE_PATTERNS = [
    ...
    # Main development style triggers (default professional mode)
    (r'\b(implement|build|code|develop|fix|refactor|optimize|debug|deploy|create|update|modify)\b', 'main')
]
    ...
    try:
        if settings_path.exists():
            with open(settings_path, 'r') as f:
                ...
                return settings.get('outputStyle', 'main')
    except Exception:
        pass
    return 'main'
...
def update_output_style(settings_path, new_style):
    ...
    current_style = settings.get('outputStyle', 'main')
