Create a `.docs/plans/[feature-name]/parallelization.md` document breaking down macOS implementation into parallel tasks using `$CLAUDE_HOME/file-templates/parallelization.template.md`.
...
Files to Create
- /path/to/NewViewController.swift
- /path/to/NewViewModel.swift
...
[Minimal implementation instructions for placeholder SwiftUI View, mock data, etc.]
```
...
Organize phases:
- Phase 1: Foundation/core setup (Models, ViewModels, Services)
- Phase 2+: Feature implementations (Views, UI components - parallel)
