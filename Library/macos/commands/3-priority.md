I need to create a `.docs/plans/[feature-name]/priority.md` document that ruthlessly prioritizes macOS features into a 3-tier system.
...
Analyze: `.claude/` exists? Recent commits? Substantial codebase (>10 Swift files)? Requirements reference existing Views?

**Auto-suggest then confirm:**
- **New macOS App**: No .claude/, few commits, minimal files → MVP focus, ruthless stubbing
- **Feature Addition**: Established app → Core feature focus, minimal changes to existing Views

Ask: "I detected this is a [new macOS app/feature addition]. Correct?"

### 3. Prioritization Strategy

**New macOS Apps (MVP Focus):**
- Ask: "What's the absolute minimum for v0.1 to deliver value to macOS users?"
- Most features → Tier 2 (stub) or Tier 3 (future)

**Feature Additions (Minimal Change):**
- Ask: "What's the minimal change needed for this macOS feature?"
- Focus on core feature without breaking existing Views or data models

### 4. Create 3-Tier System

Using `$CLAUDE_HOME/file-templates/priority.template.md`:

**Tier 1: Build Now** - Only absolute essentials for macOS feature to function
...
**Rules:**
- Be RUTHLESS, especially for new macOS apps
- Default to Tier 2 over Tier 1 when uncertain
- State WHY each item is in its tier
- Specify stub strategy for all Tier 2 items (e.g., "Placeholder SwiftUI View", "Mock data array")
