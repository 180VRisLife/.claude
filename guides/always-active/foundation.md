...
**Local Docs First**: Before implementing with external libraries, check `.docs/` for existing documentation. Local docs are LLM-optimized and faster than external lookups. If documentation is missing for a library you need, proactively suggest creating it: "I need docs for [X] - should I create them via `/utl:documentation --feature [topic]`?"
...
**Documentation**:
- `/utl:documentation` - Quick lookups and documentation generation
...
3. If missing → ask user: "Need docs for [X]. Create via `/utl:documentation --feature [topic]`?"
