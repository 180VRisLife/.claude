#!/usr/bin/env python3
"""
Hook that dynamically changes output style based on keywords in user messages for visionOS development.
...
# Define keyword patterns and their corresponding output styles for visionOS development
STYLE_PATTERNS = [
    ...
    # Sr. Swift Developer style triggers (default professional mode for visionOS)
    (r'\b(implement|build|code|develop|fix|refactor|optimize|debug|deploy|spatial|immersive|realitykit|arkit|swiftui)\b', 'Sr. Swift Developer')
]
    ...
    try:
        if settings_path.exists():
            with open(settings_path, 'r') as f:
                ...
                return settings.get('outputStyle', 'Sr. Swift Developer')
    except Exception:
        pass
    return 'Sr. Swift Developer'
...
def update_output_style(settings_path, new_style):
    ...
    current_style = settings.get('outputStyle', 'Sr. Swift Developer')
    ...
    try:
        ...
        print(f"Error updating visionOS settings: {e}", file=sys.stderr)
        return False

def detect_style_from_prompt(prompt):
    """Detect which output style to use based on prompt keywords for visionOS development."""
    ...
    if current_style != detected_style:
        ...
        if update_output_style(project_settings_path, detected_style):
            ...
            output = {
                ...
                "systemMessage": f"visionOS output style switched from '{current_style}' to '{detected_style}'"
            }
