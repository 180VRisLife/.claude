#!/usr/bin/env python3
"""
Hook that dynamically changes output style based on keywords in user messages for default development.
...
# Define keyword patterns and their corresponding output styles for default development
STYLE_PATTERNS = [
    ...
    # Sr. Developer style triggers (default professional mode)
    (r'\b(implement|build|code|develop|fix|refactor|optimize|debug|deploy|create|update|modify)\b', 'Sr. Developer')
]
    ...
    try:
        if settings_path.exists():
            with open(settings_path, 'r') as f:
                ...
                return settings.get('outputStyle', 'Sr. Developer')
    except Exception:
        pass
    return 'Sr. Developer'
...
def update_output_style(settings_path, new_style):
    ...
    current_style = settings.get('outputStyle', 'Sr. Developer')
    ...
    try:
        ...
        print(f"Error updating default settings: {e}", file=sys.stderr)
        return False

def detect_style_from_prompt(prompt):
    """Detect which output style to use based on prompt keywords for default development."""
    ...
    if current_style != detected_style:
        ...
        if update_output_style(project_settings_path, detected_style):
            ...
            output = {
                ...
                "systemMessage": f"default output style switched from '{current_style}' to '{detected_style}'"
            }
