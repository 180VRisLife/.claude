...
LOG_FILE = "/tmp/claude_default_supervisor.log"
STATE_FILE = "/tmp/claude_default_todo_hook.state"
...
def main():
    logging.info("--- Default Supervisor PostToolUse Hook Triggered (Self-Reflection Prompter) ---")
    try:
        ...
        if current_hash == last_hash:
            logging.info("Default plan has not changed. Skipping reflection prompt.")
            sys.exit(0)

        logging.info("New default plan detected. Injecting reflection prompt.")

        # Load the shared parallel execution guide
        try:
            with open(PARALLEL_GUIDE_PATH, 'r') as f:
                parallel_guide_content = f.read()
            logging.info(f"Successfully loaded default parallel guide from {PARALLEL_GUIDE_PATH}")
        except FileNotFoundError:
            logging.error(f"Default parallel guide not found at {PARALLEL_GUIDE_PATH}")
            parallel_guide_content = "Error: Could not load default parallel execution guide."
        except Exception as e:
            logging.error(f"Error loading default parallel guide: {e}")
            parallel_guide_content = f"Error loading default parallel guide: {e}"

        reflection_prompt = f"""<system-reminder>
**Parallelize the Default Plan**

The initial plan has been drafted. Now, review and apply these parallelization principles:

{parallel_guide_content}

CRITICAL ADDITIONAL REQUIREMENTS FOR DEFAULT CONTEXT:
...
Please present your analysis of parallel stages based on the guide above, then proceed with the first stage.
        ...
        logging.info("Injecting default context to trigger self-reflection and parallelization.")
        print(json.dumps(response), flush=True)

        with open(STATE_FILE, 'w') as f:
            f.write(current_hash)
        logging.info(f"Updated default state file with new hash: {current_hash}")

    except Exception as e:
        logging.exception("An unexpected error occurred in the Default Supervisor hook.")
