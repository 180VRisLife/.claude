...
If on feature branch with generic name (`feature[-/]\d{8}-\d{6}`) or name mismatches diff:
generate name (haiku), show `old → new`, ask "Rename? [Y/n/custom]".
On rename: `git branch -m old new && git push origin :old && git push -u origin new`

## Protected Branch Smart Defaults

**Protected:** `main`, `master`, `develop`, `staging`. Check triviality via `git diff --stat HEAD`.
